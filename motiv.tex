...
The advances in {\em Machine Learning (ML)} have enabled the
...
\code{Button mButton} at line 1, to \code{mButton},
\code{findViewById}, etc. at line 8, and to \code{mButton} and
\code{setOnClickListener} at line 11. In other cases, the two relevant
...
\code{View} at line 12) and a couple surrounding lines.
%
First, a few lines might not capture the relevant API elements in the
same usage. In Fig.~\ref{fig:example1}, \code{mButton} at line 1 is
far apart from \code{mButton} at line 8 and \code{mButton} at line
...
\code{onCreate}. Thus, this type of context might not help a model
